...
| `monospace_trigger` | `'[\|\x{2500}-\x{257F}]\|-{3,}\|={3,}\|\S {4,}'` | A regular expression over a row's text, trailing blanks trimmed. A row it matches is set in the configured face; any other row is set in the prose face, when the run has one (`--prose-font`). The shipped run of spaces needs something before it on the row: indentation lines up in prose by itself, so it is no reason to leave prose. A pattern that will not compile is refused in the log and nothing is put in its place. Write it as a TOML literal string (single quotes) so the backslashes stay; the file is UTF-8, so `[│┼─]` is as good as a code-point range. Live. |
